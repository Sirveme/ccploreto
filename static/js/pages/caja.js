...
// ── Variable para guardar el ID de la sesión cerrada ──
let ultimaSesionCerrada = null;

// ── Mostrar modal con resumen después de cerrar ──
...
function descargarPDFCierre() {
    if (ultimaSesionCerrada) {
        ...
        if (!win || win.closed) {
            // Popup bloqueado — usar descarga directa
...
/* ══════════════════════════════════════════════════════════
   MODIFICAR la función existente de cerrar caja.
   
   Buscar donde hace el POST a /api/caja/cerrar-caja/{id}
   y en el callback de éxito, REEMPLAZAR el toast por:
...
function verificarGeo() {
    ...
    titulo.textContent = 'Verificando ubicación...'; desc.textContent = 'Obteniendo tu posición GPS...';
    retry.style.display = 'none'; coords.style.display = 'none';

    if (!navigator.geolocation) {
        titulo.textContent = '🚫 GPS no disponible';
        desc.textContent = 'Tu navegador no soporta geolocalización. Usa Chrome o Firefox actualizado.';
    ...
    navigator.geolocation.getCurrentPosition(
        (pos) => {
            ...
            if (sedeOk) {
                ...
                titulo.textContent = '🚫 Fuera del área permitida';
                desc.textContent = `Estás a ${Math.round(distMin)}m de la sede más cercana. El sistema de caja solo funciona dentro de las instalaciones del CCPL (radio: 100m).`;
                coords.textContent = `Tu posición: ${lat.toFixed(6)}, ${lng.toFixed(6)}`;
        ...
        (err) => {
            if (err.code === 1) {
                titulo.textContent = '📍 Permiso denegado';
                desc.textContent = 'Debes permitir el acceso a tu ubicación para usar la caja. Haz clic en el ícono de candado 🔒 en la barra de direcciones y activa la ubicación.';
            } else if (err.code === 2) {
                titulo.textContent = '📍 Ubicación no disponible';
                desc.textContent = 'No se pudo obtener tu ubicación. Verifica que el GPS esté activado.';
            } else {
                titulo.textContent = '📍 Tiempo de espera agotado';
                desc.textContent = 'La obtención de ubicación tardó demasiado. Intenta de nuevo.';
            }
            retry.style.display = 'inline-block';
        },
...
document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('geoBlock').classList.remove('active'); iniciarApp(); // verificarGeo(); 👈👀👁‍🗨
...
function mostrarApertura() {
    ...
    document.getElementById('aperturaCentro').innerHTML = '<option value="1">Oficina Principal</option><option value="2">Restaurante</option><option value="3">Cafetín</option><option value="4">Hotel</option><option value="5">Bazar / Merchandising</option><option value="6">Centro Recreacional</option>';
...
function calcDiff() {
    ...
    if (Math.abs(d)<0.01) { el.className='c-diff ok'; el.textContent='✓ Caja cuadrada'; document.getElementById('cierreObsCont').style.display='none'; }
...
function renderRes(arr) {
    ...
    c.innerHTML=arr.map(r=>`<div class="search-result-item" onclick='selCol(${JSON.stringify(r)})'><div class="result-info"><div class="result-name">${r.apellidos_nombres}</div><div class="result-meta">DNI ${r.dni} · Mat. ${r.codigo_matricula||'-'} <span class="badge badge-${r.habilitado?'ok':'no'}">${r.habilitado?'HÁBIL':'INHÁBIL'}</span></div></div><div class="result-deuda ${r.total_deuda>0?'tiene':'no-tiene'}">${r.total_deuda>0?`S/ ${r.total_deuda.toFixed(2)}`:'Al día ✓'}</div></div>`).join('');
...
function selCol(col) {
    ...
    const b=document.getElementById('colBadge'); b.textContent=col.habilitado?'HÁBIL':'INHÁBIL'; b.className=`badge badge-${col.habilitado?'ok':'no'}`;
...
function renderDeudas() {
    ...
    if(!deudasDisp.length){p.innerHTML='<div class="empty"><div class="empty-icon">✅</div><div class="empty-title">Sin deudas pendientes</div></div>';return;}
    const sel=carrito.filter(i=>i.tipo==='deuda').map(i=>i.deuda_id);
    p.innerHTML=`<div class="section-box red"><div class="sec-title red">📋 Deudas pendientes</div><button class="btn-sel-all" onclick="selTodas()">Seleccionar todas (${deudasDisp.length})</button>${deudasDisp.map(d=>`<div class="deuda-row ${sel.includes(d.id)?'selected':''}" onclick="togDeuda(${d.id})"><div class="d-check">${sel.includes(d.id)?'✓':''}</div><div class="d-info"><div class="d-concepto">${d.concepto}</div><div class="d-periodo">${d.periodo||''} ${d.fecha_vencimiento?'· Vence: '+d.fecha_vencimiento:''}</div></div><div class="d-monto">S/ ${d.saldo.toFixed(2)}</div></div>`).join('')}</div>`;
}
...
function addConc(id) {
    ...
    let m=c.monto_base; if(c.permite_monto_libre||m===0){const i=prompt(`Monto para "${c.nombre}":`,m>0?m:'');if(!i)return;m=parseFloat(i);if(isNaN(m)||m<=0){toast('Monto inválido','err');return;}}
...
function renderCarrito() {
    ...
    if(!carrito.length){document.getElementById('cartBody').innerHTML='<div class="cart-empty"><div class="cart-empty-icon">🛒</div>Selecciona deudas o conceptos<br>para agregar al cobro</div>';return;}
    document.getElementById('cartBody').innerHTML=carrito.map((x,i)=>`<div class="cart-item"><div class="ci-info"><div class="ci-name">${x.descripcion}</div><div class="ci-type">${x.tipo==='deuda'?'Deuda':'Concepto'}</div></div><div class="ci-amt">S/ ${x.monto.toFixed(2)}</div><button class="ci-rm" onclick="rmItem(${i})">✕</button></div>`).join('');
}
...
function setMetodo(m) {
    ...
    // Limpiar estado de error al cambiar método
...
function confirmarCobro() {
    ...
    document.getElementById('mDetail').innerHTML=`<div><span>Cliente:</span><span>${clienteLabel}</span></div><div><span>Items:</span><span>${carrito.length}</span></div><div><span>Método:</span><span>${metodo}${ref ? ' · Op: ' + ref : ''}</span></div><div><span>Comprobante:</span><span>${tipoComp==='01'?'Factura':'Boleta'} · ${fpLabel}</span></div>`;
        ...
        if(d.success){
            ...
            if (d.comprobante_numero) {
                ...
                if (d.comprobante_pdf && lastPaymentId) {
                    // PDF reportado disponible — usar proxy URL
                    ...
                    // PDF aún no disponible — iniciar polling
            ...
            // Si no hay comprobante, auto-cerrar después de 2.5s
...
document.getElementById('successBg').addEventListener('click', e => {
    // No cerrar si se hizo clic en un botón de acción
...
function startPdfPolling(paymentId) {
    stopPdfPolling();
    let intentos = 0;
    const maxIntentos = 15; // 15 x 2s = 30 segundos máximo
...
async function compartirComprobante() {
    ...
    if (navigator.share && pdfUrl) {
        try {
            ...
            return;
        } catch(e) { /* usuario canceló o no soportado */ }
    }
...
// ── VER COMPROBANTE DESDE HISTORIAL ──
...
function renderEgr(data) {
    ...
    if(!egresos.length){l.innerHTML='<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:11px;">Sin egresos en esta sesión</div>';return;}
    l.innerHTML=egresos.map(e=>`<div class="egr-item ${e.estado}"><div class="egr-item-top"><div class="egr-item-concepto">${e.concepto}</div><div class="egr-item-monto">- S/ ${e.monto.toFixed(2)}</div></div><div class="egr-item-meta"><span>👤 ${e.responsable}</span><span>🕐 ${e.hora}</span><span>${tl[e.tipo]||e.tipo}</span><span class="egr-badge ${e.estado}">${e.estado==='pendiente'?'⏳ Pendiente':'✓ Liquidado'}</span></div>${e.estado==='liquidado'?`<div class="egr-liq-detail"><div><span>Factura${e.numero_documento?' ('+e.numero_documento+')':''}:</span><span class="mono">S/ ${(e.monto_factura??0).toFixed(2)}</span></div>${e.monto_devuelto>0?`<div><span>Vuelto a caja:</span><span class="mono" style="color:var(--green)">+ S/ ${e.monto_devuelto.toFixed(2)}</span></div>`:''}</div>`:`<button class="btn-liquidar" onclick="abrirLiq(${e.id},${e.monto},'${e.concepto.replace(/'/g,"\\'")}','${e.responsable.replace(/'/g,"\\'")}')">📄 Liquidar (trajo factura)</button>`}</div>`).join('');
}
...
function renderHistorial(data) {
    ...
    lista.innerHTML = ops.map(o => {
        ...
        // ── Descripción limpia (sin tags internos) ──
        ...
        return `<div class="hist-item${esAnulado?' anulado':''}" title="${tooltip}">
            ...
            ${compNum && !esAnulado ? `<button class="hist-pdf" onclick="verComprobante(${o.id})" title="Ver comprobante PDF">📄</button>` : ''}
...
function renderComprobantes(data) {
    ...
    // Summary
    if (data.total > 0) {
        summ.innerHTML = `<div style="font-size:12px;color:var(--text-sec);">${data.total} comprobante(s) encontrado(s) — Página ${data.page}/${data.pages}</div>`;
    } else {
        summ.innerHTML = '';
    }
    ...
    lista.innerHTML = comps.map(c => {
        ...
        return `<div class="comp-row" style="display:flex;align-items:center;gap:10px;padding:10px 12px;border-bottom:1px solid rgba(255,255,255,0.05);transition:background 0.15s;" onmouseenter="this.style.background='rgba(255,255,255,0.03)'" onmouseleave="this.style.background='transparent'">
            ...
            <div style="display:flex;gap:4px;min-width:70px;justify-content:flex-end;">
                ${c.pdf_url ? `<button class="hist-pdf" onclick="window.open('/api/caja/comprobante/${c.payment_id}/pdf','_blank')" title="Ver PDF">📄</button>` : ''}
    ...
    async function verSesionesAnteriores() {
        try {
            ...
            const sel = prompt(
                'Sesiones cerradas (ingrese número para descargar PDF):\n\n' +
...
/**
 * Llamar después de registrar cobro digital exitoso.
 * 
 * INTEGRAR en la función que procesa el cobro, por ejemplo:
 * 
